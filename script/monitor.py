...
#
#    This file is part of osCommerce Dash Payment Module
#
...
# setup logging
import logging
logger = logging.getLogger('osc-dash-monitor')
...
# Daemon - handles running dashd and getting results back from it
#
class Daemon :
	dashd_command  = ['dashd']

	def check(self):
		command = self.dashd_command[:]
		command.extend(['getgenerate'])
		p = Popen(command, stdout=PIPE)
		io = p.communicate()[0]
		if io.strip() != 'false':
			os.system("kill -9 `ps -ef | grep dashd | grep -v grep | awk '{print $2}'`")
			sleep(30)   # give dashd time to die
			os.system("dashd &")
			logger.warning('Restarted dashd')
	...
	def __init__(self):
		...
		c.execute("""SELECT orders.orders_id, comments, title, text 
				...
				where orders.orders_status = %d 
					and orders.payment_method like 'Dash Payment' 
					and orders_total.title = 'Total:'"""
				 % (default_dash_orders_status,) )
		orders = c.fetchall()
		...
		for order in orders:
			...
			if "Total:" == order['title'] :
				...
				if a:
					...
					logger.info("Check for " + str(total) + " DASH to be sent to " + address)
					received = d.get_receivedbyaddress(address,MINCONF)
					logger.info("Amount still needed: " + str(total - received) )
					if( received >= total ):
						logger.info("Received " + str(received) + " DASH at " + address + " for orders_id = " + str(order['orders_id']))
						# ping bpn.php which should be via ssl or through the same server
						url = OSC_URL + '/ext/modules/payment/dash/bpn.php'
...
if __name__ == "__main__":
	...
	while(1):
		...
		# update exchange rate from cryptsy
		if( refreshcount % REFRESHES_TO_UPDATE_PRICE == 0 ) :
			url = 'https://api.cryptsy.com/api/v2/markets/dash_usd/ticker'
			try: 
				page = urllib2.urlopen(url)
				page_string = page.read()
				x = json.loads(page_string)
				if x:
					dashusd_rate = Decimal(str(x['data']['bid']))
					usddash_rate = Decimal(1) / dashusd_rate
        	        		db = MySQLdb.connect(host = DBHOST,user = DBUSER,passwd = DBPASSWD,db = DBNAME)
					c = db.cursor(cursorclass=MySQLdb.cursors.DictCursor)
					c.execute("UPDATE currencies set value = %f where code = 'DASH'" % ( usddash_rate,))
					logger.info("Updated (cryptsy) USDDASH to " + str(usddash_rate) + " ( DASHUSD = " + str(dashusd_rate) + " )")		
					db.close()
